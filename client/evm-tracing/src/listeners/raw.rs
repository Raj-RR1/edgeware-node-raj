...
impl Listener {
	pub fn new(disable_storage: bool, disable_memory: bool, disable_stack: bool, raw_max_memory_usage: usize) -> Self {
		Self {
			disable_storage,
			disable_memory,
			disable_stack,
			...
			RuntimeEvent::Step {
				...
				if let Some(context) = self.context_stack.last_mut() {
					context.current_step = Some(Step {
						...
						self.remaining_memory_usage=self.remaining_memory_usage.and_then(|inner|inner.checked_sub(memory.data.len()));
						...
						self.remaining_memory_usage = self.remaining_memory_usage.and_then(|inner|inner.checked_sub(stack.data.len()));
						if self.remaining_memory_usage.is_none(){
						return;
						}
			...
			RuntimeEvent::StepResult {
				...
				if let Some(context) = self.context_stack.last_mut() {
					if let Some(current_step) = context.current_step.take() {
						...
						self.remaining_memory_usage = self.remaining_memory_usage.and_then(|inner|{
