...
pub use edgeware_rpc_core_trace::{FilterRequest, TraceServer};
	...
	async fn filter(self, req: FilterRequest) -> TxsTraceRes {
		...
		for block_height in block_heights {
			...
			let block_header = self
				...
				.ok_or_else(|| {
					format!("Block with height {} don't exist", block_height)
				})?;
	...
	C::Api: ApiExt<B>,
{
	...
	fn cache_block(
		...
		let block_header = client
			...
			.ok_or_else(|| {
				format!("Subtrate block {} don't exist", substrate_block_id)
			})?;
